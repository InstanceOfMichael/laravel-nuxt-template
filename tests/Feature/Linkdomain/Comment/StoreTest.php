...
class StoreTest extends TestCase
{
    ...
    public function testStoreLinkCommentAsUserAndReplyAsOtherUser()
    {
        $r = $this->actingAs($this->users[0])
            ->postJson('/linkdomains/'.$this->link->linkdomain->id.'/comments', $this->getPayload())
            ->assertStatus(201)
            ->assertJson([
        ...
        $this->actingAs($this->users[1])
            ->postJson('/linkdomains/'.$this->link->linkdomain->id.'/comments', $this->getPayload($nextComment))
            ->assertStatus(201)
            ->assertJson([
    ...
    {
        $this->postJson('/linkdomains/'.$this->link->linkdomain->id.'/comments', $this->getPayload())
            ->assertStatus(401);
    }
    ...
    public function testStoreCommentEmptyPayload()
    {
        $this->actingAs($this->users[0])
            ->postJson('/linkdomains/'.$this->link->linkdomain->id.'/comments', [])
            ->assertStatus(422)
            ->assertExactJson([
                "errors" => [
